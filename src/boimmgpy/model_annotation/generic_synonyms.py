...
def set_compound_synonyms():
    ...
    synonym_list = ["triacylglycerol", "1,2-diacyl-sn-glycerol", "1-phosphatidyl-1d-myo-inositol3-phosphate",
                    "1-phosphatidyl-1d-myo-inositol4-phosphate", "1,2-diacyl-3-o-galactosyl-sn-glycerol",
                    "digalactosyl-diacylglycerol", "monogalactosyldiacylglycerol", "digalactosyldiacylglycerol",
                    "phosphatidyl-n-dimethylethanolamine","1-phosphatidyl-n-methylethanolamine",
                    "1-phosphatidyl-choline","1-monoacylglycerol"]
    compound_list = ["triacyl-sn-glycerol", "1,2-diacyl-sn-glycerol",
                     ...
                     "1,2-diacyl-sn-glycero-3-phosphocholine","Monoacylglycerol"]
...
def set_components_synonyms():
    ...
    with driver.session() as session1:
        for i in range(len(synonym_list)):
            ...
            synonym = synonym_list[i]
            session1.run('MERGE (s: Synonym {synonym:"%s"})' % str(synonym))
            session1.run("match (l:SwissLipidsCompound),(s:Synonym) where id(l)=$sl_id and "
                         "s.synonym=$synonym merge (s)-[:is_synonym_of]->(l)", synonym=synonym,
                         sl_id=lipid_compound)
